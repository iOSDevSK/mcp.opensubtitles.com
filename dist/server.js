...
export function createOpenSubtitlesServer() {
    ...
    try {
        ...
        if (supportsSessionCredentials()) {
            tools.push({
                name: "set_api_key",
                description: "Store the user's OpenSubtitles credentials for this session so downloads work, " +
                ...
                name: "api_key_status",
                description: "Report which OpenSubtitles credentials this server is using (session, environment, or the " +
        ...
        async handleToolCall(params) {
            ...
            try {
                ...
                switch (name) {
                    ...
                    case "set_api_key":
                        if (!supportsSessionCredentials()) {
                            throw new Error("set_api_key is only available on a stdio server. Send your key with the request " +
